...
<a type="button" class="btn btn-success" href="{{route('admin.whychooseus.create')}}">Add new item</a>
<!-- /.container-fluid -->
...
<table class="table">
    ...
    <tr>
        ...
        <td><img src="{{asset($whyChooseUs->icon)}}"></td>
        <td>{{$whyChooseUs->title}}</td>
        <td>{{$whyChooseUs->content}}</td>
        <td>
            <div class="btn-group" role="group" style="gap: 2px;">
                ...
                <a href="{{route('admin.whychooseus.show', $whyChooseUs->id)}}" class="btn btn-primary mt-3">
                ...
                <form action="{{route('admin.whychooseus.destroy', $whyChooseUs->id)}}"
                      method="POST">
                    @csrf
                    @method('DELETE')
